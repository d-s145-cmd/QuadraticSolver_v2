...
static bool validateDecimalString(const std::string& s) {
    ...
    if (ePos != std::string::npos) {
        if (s.find_first_of("eE", ePos + 1) != std::string::npos) return false;
...
std::string BigDecimal::toString(int maxDigits) const {
    std::string s = toString();
    std::size_t dotPos = s.find('.');
    if (dotPos == std::string::npos) return s;

    if (s.size() - dotPos - 1 > (std::size_t)maxDigits) {
        return s.substr(0, dotPos + 1 + maxDigits);
    }
    return s;
}

std::string BigDecimal::toScientific() const {
    return toScientific(10);
}

std::string BigDecimal::toScientific(int precision) const {
    if (digits == "0") return "0.0E+0";

    std::string sign = isNegative ? "-" : "";

    std::string sciExp = addSignedStrings(exponent10, std::to_string((int)digits.size() - 1));

    std::string mantissa;
    if (digits.size() == 1) {
        mantissa = digits + ".";
        for (int i = 0; i < precision; i++) mantissa += "0";
    }
    else {
        mantissa = digits.substr(0, 1) + "." + digits.substr(1);
        int fracLen = (int)digits.size() - 1;
        if (fracLen < precision) {
            mantissa += std::string(precision - fracLen, '0');
        }
        else if (fracLen > precision) {
            mantissa = mantissa.substr(0, 2 + precision);
        }
    }

    std::string expStr = normalizeString(sciExp);
    std::string expSign = (expStr[0] == '-') ? "" : "+";
    if (expStr[0] == '-') expStr = expStr.substr(1);
...
BigDecimal addBigDecimal(const BigDecimal& a_in, const BigDecimal& b_in) {
    BigDecimal a = a_in;
    BigDecimal b = b_in;
    ...
    if (a.isNegative == b.isNegative) {
        ...
        if (cmp == 1) {
            result.digits = sub(a.digits, b.digits);
            result.isNegative = a.isNegative;
        }
        else if (cmp == -1) {
            result.digits = sub(b.digits, a.digits);
...
BigDecimal subBigDecimal(const BigDecimal& a_in, const BigDecimal& b_in) {
    BigDecimal b = b_in;
    if (!b.isZero()) b.isNegative = !b.isNegative;
    return addBigDecimal(a_in, b);
}
...
BigDecimal divBigDecimal(const BigDecimal& a, const BigDecimal& b, int precision) {
    BigDecimal result;
...
BigDecimal sqrtBigDecimal(const BigDecimal& a, int precision) {
    ...
    std::string initDigits = a.digits.substr(0, std::min((int)a.digits.size(), 2));
    if (initDigits.size() == 1) initDigits += "0";

    BigDecimal x;
    x.digits = initDigits;
    x.exponent10 = std::to_string((int)a.digits.size() / 2 + std::stoi(a.exponent10));
    x.isNegative = false;
    x.normalize();
