...
std::string sub(std::string x, std::string y) {
    ...
    for (std::size_t i = 0; i < n1; i++) {
        ...
        int digitY = (i < n2) ? y[n2 - 1 - i] - '0' : 0;
        int s;
        if (digitX - borrow < digitY) {
            s = digitX - borrow + 10 - digitY;
            borrow = 1;
        }
        else {
            s = digitX - borrow - digitY;
            borrow = 0;
        }
...
std::pair<std::string, std::string> divmod(const std::string& x, const std::string& y) {
    ...
    for (std::size_t i = 0; i < x.size(); i++) {
        ...
        while (lo <= hi) {
            int mid = (lo + hi) / 2;
            if (compareStrings(mul(y, std::to_string(mid)), current) <= 0) {
                digit = mid;
                lo = mid + 1;
            }
            else {
...
std::string powString(const std::string& base, int exp) {
    if (exp == 0) return "1";
    if (base == "0") return "0";
    if (exp == 1) return base;

    std::string half = powString(base, exp / 2);
    std::string result = mul(half, half);
